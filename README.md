...
- `core/lib/` - shared helpers (`getCurrentUser`, `requireUser`), domain errors (`errors.ts`), and `ActionResult` types (`types.ts`)
...
2. `service.ts` - business logic, auth (`requireUser()`), and plan-limit checks (`hasPermission()`)
...
4. Public routes (`/`, `/sign-in`, `/sign-up`, `/api/oauth`) allow access without a session; if a `session_token` cookie is present, the visitor is redirected to `/app` (middleware checks cookie presence only, not validity).
5. Private routes require a `session_token` cookie; missing cookies redirect to `/` (landing).
6. `app/app/layout.tsx` validates the session via `getCurrentUser()`; invalid or expired sessions redirect to `/api/auth/evict`, which clears the cookie and redirects to `/`.
7. Server actions, route handlers, and other server components resolve the current user via `getCurrentUser()` (React `cache` for per-request deduplication) or throw via `requireUser()` in service layers.
...
- Stripe Checkout success URLs hit `/api/stripe/checkout-return`, which fulfills the session (DB updates + cache revalidation) and redirects to the upgrade page; the upgrade page shows a success banner via read-only Stripe verification in `checkSubscriptionSuccess.ts`.
