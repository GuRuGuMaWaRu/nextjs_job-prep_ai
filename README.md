...
- `core/lib/` - shared helpers (`getCurrentUser`, `requireUser`), error classes (`errors.ts`), `ActionResult` (`types.ts`), and UI utilities such as `errorToast.tsx`
...
- `proxy.ts` - middleware for cookie-presence redirects and Arcjet API protection
...
2. `service.ts` - business logic, `requireUser()` auth, and plan-limit checks via `hasPermission()`
...
This separation is used in modules like `jobInfos`, `questions`, `interviews`, and `users`. Plan limits are defined in `core/data/constants.ts`; `core/features/auth/permissions.ts` exposes `hasPermission(permission, user)` plus billing helpers such as `getUserPlan()` and `getUserSubscriptionInfo()`. Feature services call those helpers directly (there are no per-feature `permissions.ts` files). Resume analysis quota and persistence live in `core/features/resumeAnalysis/` (`service.ts` + `db.ts`; the analyze API route reserves quota before streaming AI output).
...
4. Public routes (`/`, `/sign-in`, `/sign-up`, `/api/oauth`) allow access without a session; if a `session_token` cookie is present on those routes, the visitor is redirected to `/app`.
5. Other routes require a `session_token` cookie at the middleware layer; missing cookies redirect to `/` (the landing page).
6. `app/app/layout.tsx` validates the session on every `/app/**` request via `getCurrentUser()` (React `cache` + session extension). Invalid or expired sessions redirect to `/api/auth/evict`, which clears the cookie and sends the user to `/`.
7. Server actions, route handlers, and services resolve the current user via `getCurrentUser()` or throw through `requireUser()` in the service layer.
...
- `core/lib/getCurrentUser.ts` and `core/lib/requireUser.ts` — session-backed user resolution for layouts, routes, and services
- `next.config.ts`
...
- Stripe Checkout `success_url` targets `/api/stripe/checkout-return`, which fulfills the session (shared logic with webhooks), then redirects to the upgrade page. The upgrade success banner uses a read-only Stripe retrieve via `checkSubscriptionSuccess` — it does not mutate subscription state during render.
